...
本文档提供了从 Vant 3 到 Ftv 的升级指南。

### 安装 Ftv

首先你需要安装 Ftv 以及 `@vant/compat`。

`@vant/compat` 是一个兼容包，可以帮助你从 Vant 3 过渡到 Ftv。
...
从 Ftv.0 开始，将不再支持 `babel-plugin-import`，请移除项目中依赖的 `babel-plugin-import` 插件。
...
> Ftv 不再提供旧版的 DatetimePicker 组件，使用 PickerGroup 组件可以实现更灵活、更丰富的交互效果，具体用法请参考 [PickerGroup](#/zh-CN/picker-group) 组件文档。
...
为了更符合直觉，我们在 Ftv 中调整了 `Dialog` 的调用方式，将 `Dialog()` 函数重命名为 `showDialog()`，并让 `Dialog` 直接指向组件对象。
...
Ftv 中，`Toast` 组件的调用方式也进行了调整，与 `Dialog` 组件的改动一致：
...
同时，Ftv 将不再在 `this` 对象上全局注册 `$toast` 方法，这意味着 `this` 对象上将无法访问到 `$toast`。
...
Ftv 中，`Notify` 组件的调用方式也进行了调整，与 `Dialog` 组件的改动一致：
...
同时，Ftv 将不再在 `this` 对象上全局注册 `$notify` 方法，这意味着 `this` 对象上将无法访问到 `$notify`。
...
Ftv 中，`ImagePreview` 组件的调用方式也进行了调整，与 `Dialog` 组件的改动一致：
...
从 Ftv 开始，所有的事件均采用 Vue 官方推荐的**驼峰格式**进行命名。
...
目前 Vant 已经支持了基于 CSS 变量的主题定制能力，相较于 Less 定制更加灵活。因此，Ftv 将不再提供基于 Less 的主题定制方式。
