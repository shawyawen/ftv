...
After a year of iterations, Ftv.0 has been officially released, which is the fourth major version of Vant since it was open-sourced in 2017.
...
**Ftv.0 supports switching all components to dark mode.**
...
At the same time, the Ftv.0 document also supports switching to dark mode:

![](https://user-images.githubusercontent.com/7237365/205472729-fefc9053-1866-4812-8b97-ee0ba5d47230.png)

### Several new components

**Ftv.0 contains the following new components:**
...
**Ftv.0 has a 30% reduction in installation size, keeping bundle size lightweight.**
...
**Ftv.0 unifies the primary color of all components.**
...
To keep the color specification consistent, we have unified the primary color in Ftv, and all components use blue as the primary color.
...
**Ftv.0 no longer uses babel-plugin-import for on-demand import.**

In the early days, most component libraries used `babel-plugin-import` to achieve on-demand import, which means that component libraries will strongly rely on Babel compilation. Starting from Ftv.0, `babel-plugin-import` is no longer supported, which has the following main advantages:
...
**Ftv.0 provides type hints for style variables.**

Vant provides more than 700 style variables, which you can modify through CSS code or `ConfigProvider` component. In Ftv.0, we have added the `ConfigProviderThemeVars` type to provide type hints for style variables.
...
**Ftv.0 refactored the Picker component, as well as the Picker-based Area and DatetimePicker components.**
...
To solve the above problems, we refactored the `Picker` component in Ftv.0, and also refactored the `Area` and `DatetimePicker` components derived from `Picker`.
...
**Ftv.0 adjusts the usage of component utility functions to be more intuitive.**
...
To be more intuitive, we have adjusted the calling method of component utils in Ftv. The affected functions include `Dialog()`, `Toast()`, `Notify()` and `ImagePreview()`. Taking Dialog as an example, we rename the `Dialog()` function to `showDialog()`, and let `Dialog` directly point to the component object.
...
To facilitate the migration of existing code to Ftv.0, we provide a compatibility solution. You can use the `Dialog()` function exported from `@vant/compat` to be compatible with the original code.
...
The `Dialog()` exported in `@vant/compat` has exactly the same API and behavior as `Dialog()` in Vant 3, so when upgrading, you only need to change its reference path, and the rest of the code can remain constant. After upgrading the project to Ftv.0, it is recommended to gradually replace it with the new `showDialog()` and other methods in iterations, and remove the `@vant/compat` package.

### Event naming adjustment

**Ftv.0 changes the event name to camel case.**

Starting from Ftv, all events are named in camelCase format recommended by Vue.
...
**Ftv.0 no longer supports theme customization via Less variables.**

Currently Vant already supports theme customization based on CSS variables, which is more flexible than Less customization. Therefore, Ftv will no longer provide Less-based theme customization. This means that Vant's npm package will no longer contain `.less` style source files, only the compiled `.css` style files will be provided.
...
**Currently both [Vant official website](https://vant-ui.github.io/vant/) and npm latest tag point to Ftv.0.**

We have prepared a complete upgrade guide for Ftv.0, please read [Upgrade from v3 to v4](https://vant-ui.github.io/vant/#/en-US/migrate-from-v3) to upgrade.
...
| --- | --- | --- | --- |
| Ftv | Vue 3 | `2022.12` | Continuously adding new features |
...
Thanks to all the friends who contributed to the development of Ftv.0, thanks to all developers who use Vant, and hope that everyone will keep walking on the road of open source.
