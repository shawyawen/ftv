...
In order to be more intuitive, we adjusted the calling method of `Dialog` in Ftv, renamed the `Dialog()` function to `showDialog()`, and let `Dialog` directly point to the component object.
